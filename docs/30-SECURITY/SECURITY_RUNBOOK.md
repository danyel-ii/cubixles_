...
- Track swap failures with `npm run monitor:swaps` (requires `MAINNET_RPC_URL` and `contracts/deployments/mainnet.json`; override path with `CUBIXLES_DEPLOYMENT_PATH`).
