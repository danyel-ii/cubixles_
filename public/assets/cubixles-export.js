...
let payload = {};
if (scriptEl?.dataset?.export) {
  try {
    payload = JSON.parse(decodeURIComponent(scriptEl.dataset.export));
  } catch (error) {
...
const EMBEDDED_URLS = Array.isArray(payload.urls) ? payload.urls : [];
const EMBEDDED_BACKGROUND = payload.background || null;
let textures = [];
let faceTextures = [];
let bgImage = null;
let cubeSize = Number.isFinite(payload.cubeSize) ? payload.cubeSize : 220;
let rotX = Number.isFinite(payload.rotX) ? payload.rotX : -0.35;
let rotY = Number.isFinite(payload.rotY) ? payload.rotY : 0.65;
let zoom = Number.isFinite(payload.zoom) ? payload.zoom : 520;
let minZoom = Number.isFinite(payload.minZoom) ? payload.minZoom : 260;
let maxZoom = Number.isFinite(payload.maxZoom) ? payload.maxZoom : 900;
let edgePasses = [];
let lastMouse = null;
