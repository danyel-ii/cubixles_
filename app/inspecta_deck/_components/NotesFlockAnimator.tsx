...
export default function NotesFlockAnimator() {
  useEffect(() => {
    ...
    let lastRun = 0;
    const deckRoot =
      document.querySelector<HTMLElement>(".inspecta-deck-root") ?? document.body;

    const setCubeFadeDuration = (durationMs: number) => {
      const clamped = Math.max(0, durationMs);
      deckRoot.style.setProperty("--cube-fade-duration", `${clamped}ms`);
    };

    const startCubeFade = () => {
      deckRoot.classList.add("cube-fade-in");
    };
    ...
    try {
      ...
      if (!shouldRun) {
        ...
        deckRoot.classList.add("notes-docked");
    ...
    const layoutTiles = () => {
      ...
      deckRoot.classList.remove("notes-docked");
      deckRoot.classList.remove("cube-fade-in");
    ...
    return () => {
      ...
      deckRoot.classList.remove("notes-docked");
      deckRoot.classList.remove("cube-fade-in");
      deckRoot.style.removeProperty("--cube-fade-duration");
    };
  }, []);
