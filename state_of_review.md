...
The repo is now aligned on the "cubeless" name, the Farcaster manifest includes both `miniapp` and `frame` blocks, and the mint UI builds metadata with `animation_url`. Contract tests pass locally. The remaining work is primarily deployment setup (IPFS pinning, manifest assets, and onchain deployment wiring).
...
- **Contracts**: Foundry tests pass (10 total).
