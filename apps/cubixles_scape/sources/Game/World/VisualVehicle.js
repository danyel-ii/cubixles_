...
export class VisualVehicle
{
    ...
    setParts()
    {
        ...
        this.model.traverse((child) =>
        {
            ...
            for(const search of searchList)
            {
                ...
                {
                    const key = match[0]
                    if(key === 'antenna' && child.name !== 'antenna')
                        continue
                    if(typeof this.parts[key] !== 'undefined')
                        continue
                    this.parts[key] = child
                }
            }
        })
    ...
    setPaints()
    {
        ...
        this.paints.changeTo = (name = 'red') =>
        {
            ...
            if(!this.parts.bodyPainted.userData?.preventPaints)
    ...
    setCubeSkin(materials)
    {
        if(!this.parts.skinCube || !materials)
            return

        this.parts.skinCube.visible = true

        if(Array.isArray(materials))
        {
            this.parts.skinCube.material = materials
        ...
        {
            this.parts.skinCube.material = materials
            this.parts.skinCube.material.needsUpdate = true
        }
    }
    ...
    setAntenna()
    {
        const antennaObject = this.model.getObjectByName('antenna') || this.parts.antenna
        const antennaHead = this.model.getObjectByName('antennaHead')
        const antennaHeadReference = antennaObject?.getObjectByName('antennaHeadReference') || this.model.getObjectByName('antennaHeadReference')

        if(!antennaObject || !antennaHead || !antennaHeadReference)
            return

        this.antenna = {}
        this.antenna.target = new THREE.Vector3(0, 2, 0)
        this.antenna.target = new THREE.Vector3(0, 2, 0)
        this.antenna.object = antennaObject
        this.antenna.head = antennaHead
        this.antenna.headReference = antennaHeadReference
