...
export class ProjectsArea extends Area
{
    ...
    setDome()
    {
        const zoneReference = this.references.items.get('zoneBounding')?.[0]
        if(!zoneReference)
            return

        const radius = zoneReference.scale.x * 1.25
        const position = zoneReference.position.clone()

        const baseGeometry = new THREE.IcosahedronGeometry(radius, 2).toNonIndexed()
        ...
        domeGeometry.setAttribute('position', new THREE.Float32BufferAttribute(filteredPositions, 3))
        ...
        domeMesh.position.copy(position)
        domeMesh.castShadow = false
        domeMesh.receiveShadow = false
        this.game.scene.add(domeMesh)
        this.objects.hideable.push(domeMesh)

        const domeWire = new THREE.LineSegments(
            new THREE.WireframeGeometry(domeGeometry),
            new THREE.LineBasicMaterial({ color: 0xff2eb4, transparent: true, opacity: 0.55 })
        )
        domeWire.position.copy(position)
        domeWire.scale.setScalar(1.01)
        this.game.scene.add(domeWire)
        this.objects.hideable.push(domeWire)

        this.dome = {
            mesh: domeMesh,
            wire: domeWire,
            collider: this.game.objects.add(
                null,
                {
                    type: 'fixed',
                    position,
    ...
    setImages()
    {
        ...
        this.images.getResourceAndLoad = (key) =>
        {
            ...
            const isKtx = key.toLowerCase().endsWith('.ktx')
    ...
    setUrl()
    {
        ...
        this.url.update = (direction) =>
        {
            ...
            gsap.to(this.url.inner.rotation, { x: Math.PI * rotationDirection, duration: 1, delay: 0.3, ease: 'power2.in', overwrite: true, onComplete: () =>
            {
                ...
                const urlValue = this.navigation.current.url
                const displayText = urlValue ? urlValue.replace(/https?:\/\//, '') : 'Return'
