...
export class AltarArea extends Area
{
    ...
    setBeam()
    {
        ...
        const nodeSize = 256
        nodeCanvas.width = nodeSize
        nodeCanvas.height = nodeSize
        const nodeContext = nodeCanvas.getContext('2d')
        nodeContext.fillStyle = '#000000'
        nodeContext.fillRect(0, 0, nodeSize, nodeSize)
        nodeContext.font = `700 ${Math.floor(nodeSize * 0.38)}px "Amatic SC"`
        nodeContext.textAlign = 'center'
        nodeContext.textBaseline = 'middle'
        nodeContext.fillStyle = '#ffffff'
        nodeContext.fillText('node', nodeSize * 0.5, nodeSize * 0.55)

        const nodeTexture = new THREE.CanvasTexture(nodeCanvas)
        nodeTexture.colorSpace = THREE.SRGBColorSpace
        nodeTexture.flipY = false
        nodeTexture.minFilter = THREE.LinearFilter
        nodeTexture.magFilter = THREE.LinearFilter
